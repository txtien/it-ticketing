...
import { createTicketAction, deleteTicketAction, updateTicketAction } from "@/app/actions/ticket-type";
...
const TicketTable = ({ data }: { data: TicketType[] }) => {
  ...
  const handleSubmit = () => {
    if (formData.id) {
      updateTicketAction(formData as TicketType);
    } else {
      createTicketAction(formData as TicketType);
    }
    setOpenDialog(false);
    resetForm();
  };
  ...
  const openDeleteDialog = (item: TicketType) => {
  ...
  const handleDelete = () => {
    if (formData.id) {
      deleteTicketAction(formData as TicketType);
    }
  };
  return (
    <Dialog
    ...
    >
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Tạo ticket</DialogTitle>
        </DialogHeader>
